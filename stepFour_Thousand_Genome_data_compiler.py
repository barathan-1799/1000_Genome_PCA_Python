def Thousand_Genome_data_compiler(user_data_df, df_10_list, user_data_population):
  ...
  for chr_num in chr_num_list:
    ...
    user_data_df.replace(to_replace=chrom, value=chr_num, inplace=True)


  # Step 4: Replacing user's data sample IDs with their respective population abbreviation
  ...
  for column_header in list(user_data_df.columns)[4:]:
    if column_header[0:len(user_data_population)] == user_data_population:
      column_header = column_header[0:len(user_data_population)]

    column_header_list.append(column_header)

  user_data_df2.columns = column_header_list
  print("Number of samples within user's data:", user_data_df2.shape[1] - 4)
  ...
  #df_thousandG = [df_10_list[0], df_10_list[1], df_10_list[3], df_10_list[2], df_10_list[4]]
  df_thousandG = [df_10_list[0], df_10_list[1], df_10_list[2], df_10_list[3], df_10_list[4]]
  ...
  def matching_POS_extractor(df, user_data_df2):
    ...
    POS_ThousandG = [int(item) for item in list(df.iloc[:,1])]
    POS_JH = [int(item) for item in list(user_data_df2.iloc[:,1])]
  ...
  print("Removing", len(index_delete), "SNPs with POS that doesn't match between user's exome data and Thousand Genome data...")
  print("After LD filtering, there are", df_final.shape[0], "number of SNPs involving", df_final.shape[1] - 4, "individuals (samples).")

  #df_final.to_csv('1000_Genome_Final_v2.csv', index=False)
  #df_final_transposed = df_final.T
  df_final_transposed = pd.concat([df_final.T.reset_index(drop=True), pd.DataFrame(np.array(df_final.columns), columns = ["Population"])], axis=1)
  ...
  print("1000 Genome data has been compiled with user's data successfully!")
  print("=====================================================================================================================")
