# Step 12: Creating a function that filters data based on LD value
def LD_calculator(df9, LD_threshold):
  ...
  df_population_list = [df9["AFR"], df9["AMR"], df9["EUR"], df9["EAS"], df9["SAS"]]
  #df_population_list = [df_AFR]
  ...
  for df in df_population_list:
    ...
    df_P_list = [pd.DataFrame(Pa, columns = ["P_A"]), pd.DataFrame(Pb, columns = ["P_B"]),
                 pd.DataFrame(Pab, columns = ["P_AB"]), pd.DataFrame(LD, columns = ["LD"])]
    ...
    del Pab
    del df_P_list
    del index_remove_LD_one
    del index_remove_LD_two
    del index_remove_LD
    del df2
    gc.collect()
